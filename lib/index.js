    ...
    value: function startUpload(file) {
      ...
      Promise.resolve().then(function () {
        ...
        task.on('state_changed', function (snapshot) {
          ...
          return _this2.props.onProgress && _this2.props.onProgress(Math.round(progress), task);
        }, function (error) {
          return _this2.props.onUploadError && _this2.props.onUploadError(error, task);
        }, function () {
          return storageRef.child(filenameToUse).getMetadata().then(function (metadata) {
            _this2.removeTask(task);
            return _this2.props.onUploadSuccess && _this2.props.onUploadSuccess(metadata.name, task);
          });
        });
